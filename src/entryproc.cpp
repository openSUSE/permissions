...
EntryProcessor::EntryProcessor(const ProfileEntry &entry, const CmdlineArgs &args, const bool apply_changes) :
    ...
    m_ctx.subpath = entry.file.substr(args.root_path.getValue().length());
}
...
bool EntryProcessor::process(const bool have_proc) {
        ...
        if (have_proc) {
            ...
            m_ctx.fd_path = std::string("/proc/self/fd/") + std::to_string(m_ctx.fd.get());
        } else {
        ...
        if (m_euid != 0) {
            ...
            // certain limits)
            m_ctx.need_fix_ownership = false;
        }
...
bool EntryProcessor::resolveOwnership() {
    ...
    if (const auto pwd = getpwnam(m_entry.owner.c_str()); pwd != nullptr) {
        m_ctx.uid = pwd->pw_uid;
    } else if (stringToUnsigned(m_entry.owner, m_ctx.uid)) {
        // it's a numerical value, lets try it
    } else {
        good = false;
        if (m_args.verbose.isSet()) {
            std::cerr << m_ctx.subpath << ": unknown user " << m_entry.owner << ". ignoring entry." << std::endl;
        }
    }

    if (const auto grp = getgrnam(m_entry.group.c_str()); grp != nullptr) {
        m_ctx.gid = grp->gr_gid;
    } else if (stringToUnsigned(m_entry.group, m_ctx.gid)) {
        // it's a numerical value, lets try it
    } else {
        good = false;
        if (m_args.verbose.isSet()) {
            std::cerr << m_ctx.subpath << ": unknown group " << m_entry.group << ". ignoring entry." << std::endl;
        }
    }
...
bool EntryProcessor::safeOpen() {
    ...
    std::string path_rest = m_ctx.subpath;
    std::string component;

    m_ctx.traversed_insecure = false;
    m_ctx.fd.close();
    ...
    while (!is_final_path_element) {
        ...
        if (component == ".." && !altroot.empty() && m_ctx.status.sameObject(root_status))
            continue;

        {
            ...
            if (tmpfd == -1) {
                if (errno != ENOENT) {
                    std::cerr << "warning: skipping " << m_ctx.subpath << ": " << pathfd.path() << ": openat(): "
        ...
        if (!m_ctx.status.fstat(pathfd)) {
            std::cerr << "warning: skipping  " << m_ctx.subpath << ": " << pathfd.path() << ": fstat(): "
                << std::strerror(errno) << std::endl;
            return false;
        }
        ...
        if (is_parent_element) {
            ...
            if (!m_ctx.status.hasSafeOwner({m_euid})) {
                m_ctx.traversed_insecure = true;
            // if the dir is group-writable then require the root group or our
            // effective GID.
            } else if (!m_ctx.status.hasSafeGroup({m_egid})) {
                m_ctx.traversed_insecure = true;
            }
            // path is in a world-writable directory
            else if (!m_ctx.status.isLink() && m_ctx.status.isWorldWritable()) {
                m_ctx.traversed_insecure = true;
            }
        }
        ...
        if (!m_ctx.status.hasSafeOwner({m_ctx.uid,m_euid})) {
            if (is_final_path_element) {
                std::cerr << m_ctx.subpath << ": has unexpected owner (" << m_ctx.status.st_uid << "). refusing to correct due to unknown integrity." << std::endl;
                return false;
            } else {
                std::cerr << m_ctx.subpath << ": on an insecure path - " << pathfd.path() << " has different non-root owner who could tamper with the file." << std::endl;
                return false;
            }
        }

        // same goes for the group, if it is writable
        if(!m_ctx.status.hasSafeGroup({m_ctx.gid, m_egid})) {
            if (is_final_path_element) {
                std::cerr << m_ctx.subpath << ": is group-writable and has unexpected group (" << m_ctx.status.st_gid << "). refusing to correct due to unknown integrity." << std::endl;
                return false;
            }
            else {
                std::cerr << m_ctx.subpath << ": on an insecure path - " << pathfd.path() << " has different non-root group that could tamper with the file." << std::endl;
                return false;
            }
        }

        if (m_ctx.status.isLink()) {
            ...
            if (is_final_path_element)
               return false;
                ...
                std::cerr << m_ctx.subpath << ": excess link count stopping at " << pathfd.path() << "." << std::endl;
                return false;
            }
            ...
            if (!m_ctx.status.hasSafeOwner({m_euid}) || !m_ctx.status.hasSafeGroup({m_egid})) {
                std::cerr << m_ctx.subpath << ": on an insecure path - " << pathfd.path() << " has different non-root owner who could tamper with the file." << std::endl;
                return false;
            }
            ...
            if (len <= 0 || static_cast<size_t>(len) >= link.size()) {
                std::cerr << m_ctx.subpath << ": " << pathfd.path() << ": readlink(): " << std::strerror(errno) << std::endl;
                return false;
            }
        ...
        } else if (m_ctx.status.isDirectory()) {
    ...
    if (m_ctx.status.isRegular() && m_ctx.status.isWorldWritable()) {
        std::cerr << m_ctx.subpath << ": file has insecure permissions (world-writable)" << std::endl;
        return false;
    }

    m_ctx.fd.steal(pathfd);
...
bool EntryProcessor::getCapabilities() {
    ...
    if (!m_ctx.fetchCapabilities()) {
        std::cerr << m_ctx.subpath << ": " << m_ctx.caps.lastErrorText() << std::endl;
        return false;
    }
...
void EntryProcessor::printDifferences() const {
    ...
    if (m_ctx.need_fix_ownership) {
        std::cout << "wrong owner/group " << FileOwnership(m_ctx.status);
        need_comma = true;
    }

    if (m_ctx.need_fix_perms) {
        if (need_comma) {
            std::cout << ", ";
        }
        std::cout << "wrong permissions " << FileModeInt(m_ctx.status.getModeBits());
        need_comma = true;
    }

    if (m_ctx.need_fix_caps) {
        if (need_comma) {
            std::cout << ", ";
        }

        if (m_ctx.caps.hasCaps()) {
            std::cout << "wrong capabilities \"" << m_ctx.caps.toText() << "\"";
        } else {
...
bool EntryProcessor::isSafeToApply() const {
    ...
    if ((m_entry.hasCaps() || m_entry.hasSetXID()) && !m_ctx.status.isRegular() && !m_ctx.status.isDirectory()) {
        std::cerr << m_ctx.subpath << ": will only assign capabilities or setXid bits to regular files or directories" << std::endl;
        return false;
    }

    // don't give high privileges to files controlled by non-root users
    if (m_ctx.traversedInsecure()) {
        if (m_entry.hasCaps() || (m_entry.mode & S_ISUID) || ((m_entry.mode & S_ISGID) && m_ctx.status.isRegular())) {
            std::cerr << m_ctx.subpath << ": will not give away capabilities or setXid bits on an insecure path" << std::endl;
            return false;
        }
    }
...
bool EntryProcessor::applyChanges() const {
    ...
    if (m_ctx.need_fix_ownership) {
        if (::chown(m_ctx.fd_path.c_str(), m_ctx.uid, m_ctx.gid) != 0) {
            std::cerr << m_ctx.subpath << ": chown(): " << std::strerror(errno) << std::endl;
            ret = false;
        }
    }
    ...
    if (m_ctx.need_fix_perms || (m_ctx.need_fix_ownership && m_entry.hasSetXID())) {
        if (::chmod(m_ctx.fd_path.c_str(), m_entry.mode) != 0) {
            std::cerr << m_ctx.subpath << ": chmod: " << std::strerror(errno) << std::endl;
            ret = false;
        }
    }
    ...
    if (m_entry.hasCaps() || m_ctx.need_fix_caps) {
        if (m_ctx.status.isRegular()) {
            ...
            FileDesc cap_fd{::open(m_ctx.fd_path.c_str(), O_NOATIME | O_CLOEXEC | O_RDONLY)};
            if (!cap_fd.valid()) {
                std::cerr << m_ctx.subpath << ": open() for changing capabilities: " << std::strerror(errno) << std::endl;
                ret = false;
                ...
                if (errno != ENODATA || m_entry.hasCaps()) {
                    std::cerr << m_ctx.subpath << ": cap_set_fd(): " << std::strerror(errno) << std::endl;
                    ret = false;
                }
            }
        } else {
            std::cerr << m_ctx.subpath << ": cannot set capabilities: not a regular file" << std::endl;
            ret = false;
        }
    }
